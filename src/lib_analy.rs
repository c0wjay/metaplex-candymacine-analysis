...
pub mod nft_candy_machine_v2 {
    ...
    pub fn mint_nft<'info>(
        ...
        if let Some(es) = &candy_machine.data.end_settings {
            ...
            //* Rust 프로그래밍 언어 공식 docs Chap.6 참고하면 좋음.
        let mut remaining_accounts_counter: usize = 0;
        if let Some(gatekeeper) = &candy_machine.data.gatekeeper {  //* rust if let some 문법 참고. candy_machine.data.gatekeeper의 값은 option형으로 선언되어 있는데, 만약 값이 있다면 (None이 아니라면) Some(something) 으로 둘러싸진 값에서 something을 gatekeeper 변수에 빌려줌. 하위 scope부터 gatekeeper는 candy_machine.data.gatekeeper의 값과 같다고 생각하면 된다.
            ...
            >(*gateway_token_info.data.borrow())?;  //* https://github.com/identity-com/on-chain-identity-gateway, https://docs.rs/crate/solana-gateway/0.1.1
            ...
            let expire_time = gateway_token
                ...
                as i64; // *https://docs.rs/solana-gateway/0.1.1/solana_gateway/state/struct.GatewayToken.html
        ...
        if let Some(ws) = &candy_machine.data.whitelist_mint_settings {
            ...
            match assert_is_ata(whitelist_token_account, &payer.key(), &ws.mint) {
                Ok(wta) => {    //* uilts.rs에서 whitelist_token_acount -> assert_initialized -> wta
                    if wta.amount > 0 {
...
pub struct MintNFT<'info> {
    #[account(
    mut,
    has_one = wallet    //* imfeld.dev/writing/starting_with_solana_part02 참고. 
    )]
    candy_machine: Account<'info, CandyMachine>,
